...
class BoreholeTest(unittest.TestCase):
    ...
    def test_interval_dataset(self):
        ...
        dataset = self.borehole.add_interval_dataset("geology", from_depths, to_depths)
        dataset.add_property(ROCK_TYPE, rock_type)
        self.assertTrue(all(from_depths == self.borehole.interval_dataset["geology"].from_depths))
        self.assertTrue(all(to_depths == self.borehole.interval_dataset["geology"].to_depths))
        self.assertEquals(["SC", "FE"], self.borehole.interval_dataset["geology"].properties["rock"].values[-1])
    ...
    def test_point_dataset(self):
        ...
        self.assertTrue(all(depths == self.borehole.point_dataset["samples"].depths))
        self.assertEquals(densities, self.borehole.point_dataset["samples"].properties["d"].values)
    ...
    def test_interval_dataset_wrong_property_size(self):
        ...
        self.assertRaises(AssertionError,
                          lambda: pybh.IntervalDataSet("test", [1, 3], [2, 4]).add_property(DENSITY, [1.3]))

    def test_point_dataset_depths_empty(self):
        """Test that empty point depths raises an AssertionError"""
        self.assertRaises(AssertionError, lambda: pybh.PointDataSet("test", []))

    def test_sampling_dataset_depths_decreasing(self):
        """Test that decreasing point depths raises an AssertionError"""
        self.assertRaises(AssertionError, lambda: pybh.PointDataSet("test", [2, 1]))

    def test_sampling_dataset_wrong_property_size(self):
        """Test that point property with wrong number of values raises an AssertionError"""
        self.assertRaises(AssertionError,
                          lambda: pybh.PointDataSet("test", [1, 2]).add_property(DENSITY, [1.3]))
