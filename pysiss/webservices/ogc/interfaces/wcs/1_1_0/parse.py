""" file: parse.py (pysiss.webservices.ogc.interfaces.wcs.1_1_0)

    description: Parsing functions for information from a WCS1.1.0
        webservice
"""
...
def get_capabilities(response):
    """ Parse a get_capabilities response from a WCS 1.1.0 webservice
    """
    # Parse XML response
    ...
    layers = [e.text() for e in capabilities.xpath('//wcs:identifier')]
...
def describe_coverage(response):
    """ Parse a describeCoverage response from a WCS 1.1.0 webservice
    """
    pass

def get_coverage(response):
    """ Parse an XML response from a WCS 1.1.0 webservice
    """
    pass
